...
def to_infinity(value):
    ...
    elif value == -infinite: value = '-Infinity'
...
def minimax_traverse_output(node, depth, value):
    ...
    line = node + ',' + str(depth) + ',' + value
    print >> traverse_log_output, line
...
def minimax(node, cur, depth, state, player, max_flag):
    "minimax algorithm"
    best_value = -infinite
    if max_flag == False: best_value = infinite
...
if task != 4: # part1
    ...
    if task == 1:
        ...
        print >> traverse_log_output, 'Node,Depth,Value'
        target_state = minimax('root', 0, depth, init_state, player, True)[1]
    elif task == 3:
        traverse_log_output = open('traverse_log.txt', 'w')
        print >> traverse_log_output, 'Node,Depth,Value,Alpha,Beta'
        target_state = alphabeta('root', 0, depth, init_state, player, -infinite, infinite, True)[1]
    ...
    while is_full(state) == False:
        ...
        if algo == 1:
            ...
            state == alphabeta('root', 0, depth, state, player, -infinite, infinite, True)[1]
